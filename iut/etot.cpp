...
bool ntegerr(const QString& str) {
    QRegularExpression re("^-?\\d*(\\.\\d+)?$");
    QRegularExpressionMatch match = re.match(str);
    return match.hasMatch();
}
void ETOT::on_ok_clicked()
{
    QString adr = ui->adress->text();
    QString sourseamount  = ui->amount->text();
    if((ntegerr(sourseamount)) && adr!=""){
        QString email = em; // get from client
        QString source_coin = "eth"; // get from user(همون جایی که کاربر وارد میکنه یه کاری بکن که مشخص بشه چه کوینی رو انتخاب کرده)
        QString destination_coin = "trx"; // get from user(همون جایی که کاربر وارد میکنه یه کاری بکن که مشخص بشه چه کوینی رو انتخاب کرده)
        QString destination_address = adr; //get from user
        float source_price,source_amount, destination_price;
        source_price  = 3125.12; //get from api
        destination_price = 0.1347; //get from api
