...
void ETOB::on_pushButton_clicked()
{
    hide();
    trade * j;
    j =new trade(this);
...
bool iintegerr(const QString& str) {
    QRegularExpression re("^-?\\d*(\\.\\d+)?$");
    QRegularExpressionMatch match = re.match(str);
    return match.hasMatch();
}
void ETOB::on_ok_clicked()
{
    QString adr = ui->adress->text();
    QString sourseamount  = ui->amount->text();
    if((iintegerr(sourseamount)) && adr!=""){
        ...
        destination_price = 57669; //get from api
